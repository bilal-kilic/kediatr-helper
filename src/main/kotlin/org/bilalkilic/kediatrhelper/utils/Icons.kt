package org.bilalkilic.kediatrhelper.utils

import com.intellij.openapi.util.IconLoader
import javax.swing.Icon

object Icons {
    @JvmField
    val kediatrGutter: Icon = IconLoader.getIcon("/icons/kediatr-gutter-icon.png", Icons::class.java)
    val createNewHandlerGutter: Icon = IconLoader.getIcon("/icons/create-new-handler-icon.png", Icons::class.java)
    val navigateToHandlerGutter: Icon = IconLoader.getIcon("/icons/navigate-to-handler-icon.png", Icons::class.java)
    // val kediatrToolbar: Icon = IconLoader.getIcon("/icons/kediatr-toolbar-icon.png", Icons::class.java)
}
